...
cd ~/robot_worlds_ws && source /opt/ros/humble/setup.bash && source install/setup.bash
ros2 launch robot_worlds tb3_robot_state_publisher.launch.py

# Terminal 3 — Autonomous node
cd ~/braitenberg_ws && source /opt/ros/humble/setup.bash && source install/setup.bash
...
| GAIN | Behavior |
|------|----------|
| 0.3 | Bold — gets close to obstacles, moves faster |
| 0.8 | Balanced — recommended setting |
| 1.5 | Cautious — reacts early, moves slower |
